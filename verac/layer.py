...
class VeraLayer(BaseTunerLayer):
    ...
    def update_layer(
        ...
        # if not self.printed_vera_A:
        #     # Print and calculate norm for vera_A
        #     vera_A_np = vera_A['default'].numpy()
        #     rounded_vera_A_np = np.round(vera_A_np, decimals=2)
        #     np.set_printoptions(precision=2, suppress=True)
        #     print(rounded_vera_A_np)
        #     np.savetxt("rounded_vera_A.csv", rounded_vera_A_np, delimiter=",")
        #     norm_A = np.linalg.norm(vera_A_np)
        #     print("Chuẩn norm của vera_A là:", norm_A)

        #     # Set the flag to True to indicate vera_A has been printed and norm calculated
        # self.printed_vera_A = True
...
class Linear(nn.Linear, VeraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.update_layer(adapter_name, vera_A, vera_B, r, vera_dropout, init_vera_weights, d_initial=d_initial, c_initial=c_initial)
    ...
    def forward(self, x: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                # print(vera_A.size())
                # print(vera_B.size())
                result += lambda_b * F.linear(lambda_d * F.linear((dropout(x) * lambda_c), vera_A), vera_B) 
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.update_layer_embedding(
            adapter_name, vera_A, vera_B, r, vera_dropout, init_vera_weights, d_initial=d_initial, c_initial=c_initial
        )
    ...
    def forward(self, x: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                result += lambda_b * (after_A @ vera_B.T) * lambda_c
